<?php
/**
 * ProfDef REST: Mark existing person as presenter
 * Endpoint: PUT /wp-json/profdef/v2/member/mark_presenter
 * Purpose: Flip beta_2.person.presenter from 0 to 1 for an existing person row.
 */
...
add_action(
    ...
    function () {
        register_rest_route(
            'profdef/v2',
            '/member/mark_presenter',
            [
                'methods'             => [ 'PUT', 'POST' ],
                'permission_callback' => 'pd_presenters_permission',
                'callback'            => 'pd_member_mark_presenter',
                'args'                => [
                    'person_id' => [
...
/**
 * Handler: mark an existing person as presenter (presenter = 1).
 *
 * @param WP_REST_Request $request
 *
 * @return WP_REST_Response|WP_Error
 */
function pd_member_mark_presenter( WP_REST_Request $request ) {
    ...
    try {
        ...
        return new WP_Error(
            'aslta_remote_error',
            'Failed to update presenter flag via remote API.',
            [
                'status' => 500,
    ...
    if ( $result['status'] < 200 || $result['status'] >= 300 ) {
        return new WP_Error(
            ...
            'Remote presenter update endpoint returned an HTTP error.',
            [
                'status' => 500,
