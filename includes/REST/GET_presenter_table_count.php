<?php
/**
 * ProfDef REST: GET_presenter_table_count
 * Endpoint: GET /wp-json/profdef/v2/presenters/ct
 * Purpose: Return total count of presenters from beta_2.GET_presenters_table_count view (via signed API).
 *
 * Returns:
 * { "count": <int> }
 */
...
add_action( 'rest_api_init', function () {
    register_rest_route(
        ...
        [
            ...
            'callback'            => 'pd_get_presenters_table_count',
        ]
    );
} );

function pd_get_presenters_table_count( WP_REST_Request $request ) {
    try {
        $schema = defined( 'PD_DB_SCHEMA' ) ? PD_DB_SCHEMA : 'beta_2';
        $sql    = sprintf( 'SELECT * FROM %s.GET_presenters_table_count;', $schema );
        ...
        return new WP_Error(
            'aslta_remote_error',
            'Failed to query presenters count via remote API.',
    ...
    if ( $result['status'] < 200 || $result['status'] >= 300 ) {
        return new WP_Error(
            ...
            'Remote presenters count endpoint returned an HTTP error.',
            [
                'status' => 500,
    ...
    if ( json_last_error() !== JSON_ERROR_NONE ) {
        return new WP_Error(
            'aslta_json_error',
            'Failed to decode presenters count JSON response.',
